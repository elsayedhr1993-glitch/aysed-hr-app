...
const replacement = `    if (leaveRecord) {
      ...
      const currentAvailableBalance = netAvailable; 
      const snapshotAvailable = leaveRecord.totalAvailableBalance ?? (leavePaidDays + currentAvailableBalance);
...
content = content.replace(
  /    if \(leaveRecord && leaveRecord\.status === 'APPROVED'\) \{[\s\S]*?    \} else \{/,
  replacement
);
